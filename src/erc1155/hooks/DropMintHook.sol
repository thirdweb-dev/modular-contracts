...
contract DropMintHook is IClaimCondition, IFeeConfig, ERC1155Hook {
  ...
  mapping(address => mapping(uint256 => ClaimCondition))
    private _claimCondition;
  ...
  function verifyClaim(
    ...
    uint256 supplyClaimedByWallet = getSupplyClaimedByWallet(
      _token,
      _id,
      _claimer
    );
  ...
  function getSupplyClaimedByWallet(
    ...
    return
      _supplyClaimedByWallet[_conditionId[_token][_id]][
        keccak256(abi.encode(_claimer, _token, _id))
      ];
  }
  ...
  function beforeMint(
    ...
    _supplyClaimedByWallet[_conditionId[token][_id]][
      keccak256(abi.encode(_claimer, token, _id))
    ] += _quantity;
  ...
  function setClaimCondition(
    ...
    if (_resetClaimEligibility) {
      ...
      targetConditionId = keccak256(
        abi.encodePacked(msg.sender, block.number, _id)
      );
    }
