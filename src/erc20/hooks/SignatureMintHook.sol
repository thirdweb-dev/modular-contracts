...
contract SignatureMintHook is IFeeConfig, IMintRequestERC20, EIP712, ERC20Hook {
     ...
     */
    function verify(address _token, MintRequestERC20 memory _req, bytes memory _signature)
        public
        view
        ...
        returns (MintParams memory mintParams)
    {
        ...
        (MintRequestERC20 memory req, bytes memory signature) = abi.decode(_encodedArgs, (MintRequestERC20, bytes));
    ...
    function _recoverAddress(MintRequestERC20 memory _req, bytes memory _signature) internal view returns (address) {
    ...
    function _processRequest(address _token, MintRequestERC20 memory _req, bytes memory _signature) internal {
        bool success;
        address signer;
