...
library RoyaltyStorage {
    ...
    bytes32 public constant ROYALTY_STORAGE_POSITION =
        keccak256(abi.encode(uint256(keccak256("token.royalty")) - 1)) &
    ...
    BeforeBatchTransferCallbackERC1155
{
        ...
        returns (ModuleConfig memory config)
    {
        ...
        config.callbackFunctions[0] = CallbackFunction(
            this.beforeTransferERC1155.selector
        );
        config.callbackFunctions[1] = CallbackFunction(
            this.beforeBatchTransferERC1155.selector
        );
        ...
        config.fallbackFunctions[5] = FallbackFunction({
            selector: this.getTransferValidator.selector,
            permissionBits: 0
        });
        config.fallbackFunctions[6] = FallbackFunction({
        ...
        config.fallbackFunctions[7] = FallbackFunction({
    ...
    function beforeTransferERC1155(
        address from,
        address to,
        uint256 _id,
        uint256 _value
    ...
    function royaltyInfo(
        ...
        (
            ...
            uint16 overrideBps
        ) = getRoyaltyInfoForToken(_tokenId);
        (address defaultRecipient, uint16 defaultBps) = getDefaultRoyaltyInfo();

        receiver = overrideRecipient == address(0)
            ? defaultRecipient
            : overrideRecipient;
    ...
    function getDefaultRoyaltyInfo() public view returns (address, uint16) {
        RoyaltyInfo memory defaultRoyaltyInfo = RoyaltyStorage
            .data()
            .defaultRoyaltyInfo;
    ...
    function setRoyaltyInfoForToken(
        ...
        RoyaltyStorage.data().royaltyInfoForToken[_tokenId] = RoyaltyInfo({
    ...
    function _setDefaultRoyaltyInfo(
        ...
        RoyaltyStorage.data().defaultRoyaltyInfo = RoyaltyInfo({
    ...
    function _royaltyStorage()
        internal
        pure
        returns (RoyaltyStorage.Data storage)
    {
