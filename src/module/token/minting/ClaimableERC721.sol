...
contract ClaimableERC721 is ModularModule, EIP712, BeforeMintCallbackERC721, IInstallationCallback {
    ...
    function getModuleConfig() external pure override returns (ModuleConfig memory config) {
        ...
        config.fallbackFunctions[4] =
            FallbackFunction({selector: this.setMintFeeManager.selector, permissionBits: Role._MANAGER_ROLE});
    ...
    function onInstall(bytes calldata data) external {
        (address primarySaleRecipient, address mintFeeManager) = abi.decode(data, (address, address));
    ...
    function _distributeMintPrice(address _owner, address _currency, uint256 _price) internal {
        ...
        if (_currency == NATIVE_TOKEN_ADDRESS) {
            ...
            uint256 platformFeeBps = MintFeeManager(_mintFeeManager).getTokenMintFee(address(this));
            uint256 platformFeeAmount = (_price * platformFeeBps) / PLATFORM_FEE_DENOMINATOR;
