...
import {MintFeeManager} from "../../../MintFeeManager.sol";
    ...
    IInstallationCallback
{
    ...
    /// @dev Emitted when the minter is not in the allowlist.
    error ClaimableMintFeeManagerNotSet();

    /*//////////////////////////////////////////////////////////////
                                CONSTANTS
    //////////////////////////////////////////////////////////////*/

    bytes32 private constant TYPEHASH_CLAIMABLE_ERC721 =
        keccak256(
    ...
    address private _mintFeeManager;
        ...
        returns (ModuleConfig memory config)
    {
        ...
        config.fallbackFunctions[4] = FallbackFunction({
            selector: this.setMintFeeManager.selector,
            permissionBits: Role._MANAGER_ROLE
        });
        config.fallbackFunctions[4] = FallbackFunction({
    ...
    function beforeMintERC721(
        ...
        ClaimParamsERC721 memory _params = abi.decode(
            _data,
            (ClaimParamsERC721)
        );
    ...
    function onInstall(bytes calldata data) external {
        (address primarySaleRecipient, address mintFeeManager) = abi.decode(
            data,
            (address, address)
        );
        _claimableStorage().saleConfig = SaleConfig(primarySaleRecipient);
        _claimableStorage().mintFeeManager = mintFeeManager;
    }
    ...
    function encodeBytesBeforeMintERC721(
        ClaimParamsERC721 memory params
    ) external pure returns (bytes memory) {
    ...
    /// @notice Sets the mint fee manager address
    function setMintFeeManager(address mintFeeManager) external {
        _claimableStorage().mintFeeManager = mintFeeManager;
    }
    ...
    function _validateClaimRequest(
        ...
        if (
            _req.quantity > _claimableStorage().claimCondition.availableSupply
        ) {
            revert ClaimableOutOfSupply();
        }
    ...
    function _distributeMintPrice(
        ...
        if (_claimableStorage().mintFeeManager == address(0)) {
            revert ClaimableMintFeeManagerNotSet();
        }
        ...
        if (_currency == NATIVE_TOKEN_ADDRESS) {
            ...
            uint256 platformFeeBps = MintFeeManager(_mintFeeManager)
                .getTokenMintFee(address(this));
            uint256 platformFeeAmount = (_price * platformFeeBps) /
