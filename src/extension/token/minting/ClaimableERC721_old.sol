...
contract ClaimableERC721 is ModularExtension, EIP712, BeforeMintCallbackERC721, IInstallationCallback {
    ...
    struct ClaimParamsERC721 {
        ...
        address currency;
        uint256 pricePerUnit;
        AllowlistProof recipientAllowlistProof;
    }
    ...
    function setClaimCondition(ClaimCondition memory _claimCondition, bool _resetClaimEligibility) external {
        bytes32 targetConditionId = _claimableStorage().conditionId;
        uint256 supplyClaimedAlready = _claimableStorage().claimCondition.supplyClaimed;
        ...
        if (supplyClaimedAlready > _claimCondition.availableSupply) {
            revert ClaimableOutOfSupply();
        }

        _claimableStorage().claimCondition = _claimCondition;
        _claimableStorage().conditionId = targetConditionId;
    }
    ...
    function _validateClaimCondition(
        ...
        bytes32 conditionId = _claimableStorage().conditionId;
        
        uint256 claimLimit = condition.quantityLimitPerWallet;
        uint256 claimPrice = condition.pricePerUnit;
        ...
        if (isOverride) {
            claimLimit = _allowlistProof.quantityLimitPerWallet != 0
                ? _allowlistProof.quantityLimitPerWallet
                : claimLimit;
            claimPrice = _allowlistProof.pricePerUnit != type(uint256).max
                ? _allowlistProof.pricePerUnit
                : claimPrice;
            claimCurrency = _allowlistProof.pricePerUnit != type(uint256).max && _allowlistProof.currency != address(0)
                ? _allowlistProof.currency
                : claimCurrency;
        }

        uint256 _supplyClaimedByWallet = _claimableStorage().supplyClaimedByWallet[conditionId][_claimer];

        if (block.timestamp < condition.startTimestamp || condition.endTimestamp <= block.timestamp) {
            revert ClaimableOutOfTimeWindow();
        }

        if (_currency != claimCurrency || _pricePerUnit != claimPrice) {
            revert ClaimableIncorrectPriceOrCurrency();
        }

        if (_amount == 0 || (_amount + _supplyClaimedByWallet > claimLimit)) {
            revert ClaimableExceedLimit();
        }

        if (_amount + condition.supplyClaimed > condition.availableSupply) {
            revert ClaimableOutOfSupply();
        }

        _claimableStorage().claimCondition.supplyClaimed += _amount;
        _claimableStorage().supplyClaimedByWallet[conditionId][_claimer] += _amount;
