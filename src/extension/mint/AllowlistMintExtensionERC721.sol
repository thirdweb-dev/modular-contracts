...
import {AllowlistMintExtensionERC721Storage} from "../../storage/extension/mint/AllowlistMintExtensionERC721Storage.sol";

contract AllowlistMintExtensionERC721 is IFeeConfig, ERC721Extension {
    /*//////////////////////////////////////////////////////////////
                               STRUCTS
    //////////////////////////////////////////////////////////////*/
    ...
    function getNextTokenIdToMint(address _token) external view returns (uint256) {
        return AllowlistMintExtensionERC721Storage.data().nextTokenIdToMint[_token];
    }
    ...
    function getFeeConfigForToken(address _token, uint256 _id) external view returns (FeeConfig memory) {
        return AllowlistMintExtensionERC721Storage.data().feeConfig[_token][_id];
    }

    /// @notice Returns the fee config for a token.
    function getDefaultFeeConfig(address _token) external view returns (FeeConfig memory) {
        return AllowlistMintExtensionERC721Storage.data().feeConfig[_token][type(uint256).max];
    }
    ...
    function getClaimCondition(address _token) external view returns (ClaimCondition memory) {
        return AllowlistMintExtensionERC721Storage.data().claimCondition[_token];
    }
