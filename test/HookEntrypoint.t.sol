...
import { ERC721Extension, AllowlistNewExtension } from "src/extension/mint/AllowlistNewExtension.sol";
 ...
 */
contract HookEntrypointTest is Test {
    /*//////////////////////////////////////////////////////////////
                                SETUP
    //////////////////////////////////////////////////////////////*/
    ...
    AllowlistNewExtension public simpleClaimExtension;
    ...
    function setUp() public {
        ...
        extensionProxyAddress = cloneFactory.deployDeterministicERC1967(
            address(new AllowlistNewExtension()),
            abi.encodeWithSelector(AllowlistNewExtension.initialize.selector, platformAdmin),
            bytes32("salt")
        );
        simpleClaimExtension = AllowlistNewExtension(extensionProxyAddress);
        ...
        vm.label(extensionProxyAddress, "AllowlistNewExtension");
        vm.label(platformAdmin, "Admin");
        vm.label(platformUser, "Developer");
        vm.label(claimer, "Claimer");

        // Developer installs `AllowlistNewExtension` extension
        vm.startPrank(platformUser);
        ...
        AllowlistNewExtension.ClaimCondition memory condition = AllowlistNewExtension.ClaimCondition({
            price: pricePerToken,
            availableSupply: availableSupply,
            allowlistMerkleRoot: root
        });

        bytes memory setClaimConditionCalldata = abi.encodeWithSelector(
            AllowlistNewExtension.setClaimCondition.selector,
            condition
        );
        erc721.hookFunctionWrite(erc721.BEFORE_MINT_FLAG(), 0, setClaimConditionCalldata);

        AllowlistNewExtension.FeeConfig memory feeConfig;
        feeConfig.primarySaleRecipient = platformUser;
        feeConfig.platformFeeRecipient = address(0x789);
        feeConfig.platformFeeBps = 100; // 1%

        bytes memory setDefaultFeeConfigCalldata = abi.encodeWithSelector(
            AllowlistNewExtension.setDefaultFeeConfig.selector,
            feeConfig
        );
        erc721.hookFunctionWrite(erc721.BEFORE_MINT_FLAG(), 0, setDefaultFeeConfigCalldata);
    ...
    function test_mintOneToken() public {
        ...
        AllowlistNewExtension.ClaimCondition memory condition = simpleClaimExtension.getClaimCondition(address(claimContract));
    ...
    function test_performUpgrade() public {
        ...
        address newImpl = address(new AllowlistNewExtension());
