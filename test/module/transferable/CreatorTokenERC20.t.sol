...
contract CreatorTokenERC20Test is Test {
    ...
    function test_state_setTransferValidator() public {
        assertEq(
            CreatorTokenERC20(address(core)).getTransferValidator(),
            address(0)
        );

        // set transfer validator
        vm.prank(owner);
        CreatorTokenERC20(address(core)).setTransferValidator(
            address(mockTransferValidator)
        );
        assertEq(
            CreatorTokenERC20(address(core)).getTransferValidator(),
    ...
    function test_revert_setTransferValidator_invalidContract() public {
        ...
        vm.prank(owner);
        vm.expectRevert(
            CreatorTokenERC20.InvalidTransferValidatorContract.selector
        );
