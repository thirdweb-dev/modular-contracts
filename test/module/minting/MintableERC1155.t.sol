...
import {MintableERC1155, MintableStorage} from "src/module/token/minting/MintableERC1155.sol";
...
contract MintableERC1155Test is Test {
    ...
    MintableERC1155 public moduleImplementation;
    MintableERC1155 public installedModule;
    ...
    function signMintRequest(
        MintableERC1155.MintRequestERC1155 memory _req,
        uint256 _privateKey
    ...
    function test_mint_state() public {
        ...
        MintableERC1155.MintRequestERC1155 memory mintRequest = MintableERC1155
            .MintRequestERC1155({
                tokenId: 0,
        ...
        bytes memory sig = signMintRequest(
            mintRequest,
            permissionedActorPrivateKey
        );

        MintableERC1155.MintParamsERC1155 memory params = MintableERC1155
            .MintParamsERC1155(mintRequest, sig, "");
        ...
        assertEq(
            core.balanceOf(address(0x123), mintRequest.tokenId),
            mintRequest.quantity
        );

        uint256 salePrice = mintRequest.quantity * mintRequest.pricePerUnit;
        uint256 platformFeeAmount = (salePrice * platformFeeBps) /
            platformFeeDenominator;
        uint256 primarySaleAmount = salePrice - platformFeeAmount;
        assertEq(
            tokenRecipient.balance,
            balBefore - salePrice,
            "Token recipient balance after mint"
        );
        assertEq(
            saleRecipient.balance,
            primarySaleAmount,
            "Sale recipient balance after mint"
        );
        assertEq(
            platformFeeRecipient.balance,
            platformFeeAmount,
            "Platform fee recipient after mint"
        );
    }
    ...
    function test_mint_revert_unableToDecodeArgs() public {
        ...
        core.mint{value: mintRequest.quantity * mintRequest.pricePerUnit}(
            ...
            mintRequest.tokenId,
            mintRequest.quantity,
            abi.encode(bytes("random mixer"), params)
        );
    }
    ...
    function test_mint_revert_requestInvalidRecipient() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MintableERC1155.MintableRequestMismatch.selector
            )
        );
        core.mint{value: mintRequest.quantity * mintRequest.pricePerUnit}(
    ...
    function test_mint_revert_requestBeforeValidityStart() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MintableERC1155.MintableRequestOutOfTimeWindow.selector
            )
        );
        core.mint{value: mintRequest.quantity * mintRequest.pricePerUnit}(
            mintRequest.recipient,
    ...
    function test_mint_revert_requestUidReused() public {
        ...
        assertEq(
            core.balanceOf(mintRequest.recipient, mintRequest.tokenId),
            mintRequest.quantity
        );
        ...
        bytes memory sigTwo = signMintRequest(
            mintRequestTwo,
            permissionedActorPrivateKey
        );

        MintableERC1155.MintParamsERC1155 memory paramsTwo = MintableERC1155
            .MintParamsERC1155(mintRequestTwo, sigTwo, "");

        vm.expectRevert(
            abi.encodeWithSelector(
                MintableERC1155.MintableRequestUidReused.selector
            )
        );
        core.mint(
            mintRequestTwo.recipient,
            mintRequestTwo.tokenId,
            mintRequestTwo.quantity,
            abi.encode(paramsTwo)
        );
    }
    ...
    function test_mint_revert_requestUnauthorizedSigner() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MintableERC1155.MintableRequestUnauthorized.selector
            )
        );
        core.mint{value: mintRequest.quantity * mintRequest.pricePerUnit}(
            mintRequest.recipient,
    ...
    function test_mint_revert_noPriceButNativeTokensSent() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                MintableERC1155.MintableIncorrectNativeTokenSent.selector
            )
        );
        core.mint{value: 1 ether}(
            mintRequest.recipient,
            mintRequest.tokenId,
            mintRequest.quantity,
            abi.encode(params)
