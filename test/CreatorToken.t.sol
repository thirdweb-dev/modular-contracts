...
contract CreatorTokenERC721Test is Test {
    ...
    function test_state_setTransferValidator() public {
        assertEq(core.getTransferValidator(), address(0));

        // set transfer validator
        vm.prank(owner);
        core.setTransferValidator(address(mockTransferValidator));
        assertEq(core.getTransferValidator(), address(mockTransferValidator));

        // set transfer validator back to zero address
        vm.prank(owner);
        core.setTransferValidator(address(0));
        assertEq(core.getTransferValidator(), address(0));
    }
    ...
    function test_revert_setTransferValidator_invalidContract() public {
        ...
        core.setTransferValidator(address(11111));
    }
