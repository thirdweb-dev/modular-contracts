...
contract SuperChainERC20SetupScript is Script {
    ...
    function deployDeterministic(bytes32 salt, bytes memory creationCode, bytes memory encodedArgs)
        public
        returns (address)
    {
        ...
        assembly {
            deployedAddress := create2(0, add(creationCodeWithArgs, 0x20), mload(creationCodeWithArgs), salt)
        }
    ...
    function run() external {
        ...
        address superchainBridge =
            deployDeterministic(salt, type(SuperChainBridge).creationCode, abi.encode(L2_TO_L2_CROSS_DOMAIN_MESSENGER));
